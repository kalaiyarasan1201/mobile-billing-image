...
import { View, Text, StyleSheet, FlatList, SafeAreaView, Alert, Platform } from 'react-native';
...
export default function HistorySalesScreen() {
  ...
  bills.forEach(bill => {
    ...
    if (bill.date === today && !hiddenSections.includes('daily')) {
      dailySales += bill.total;
      dailyBills.push(bill);
    }
    
    if (billDateObj.getMonth() === currentMonth && billDateObj.getFullYear() === currentYear && !hiddenSections.includes('monthly')) {
      monthlySales += bill.total;
      monthlyBills.push(bill);
    }
    
    if (billDateObj.getFullYear() === currentYear && !hiddenSections.includes('yearly')) {
